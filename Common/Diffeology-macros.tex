...
%% ---------------------------------------------------------------------------
%% Naming Convention for Semantic Macros:
%% ---------------------------------------------------------------------------
%% - Number fields/rings: \fieldX, \ringX (lowercase prefix)
%% - Groups: \GL, \Diff, \Hom (all caps)
%% - Operators: \Proj, \Id, \Der (capitalized)
%% - Spaces: \Torus, \Sph (capitalized)
%% - Calligraphic variants: \cTorus, \cM (lowercase 'c' prefix)
%% - Parentheses macros: \FundamentalGroup(...) for natural reading
%% ---------------------------------------------------------------------------
...
%% \BigMid, \BiggMid : Large "such that" delimiters for set definitions
%% Usage: $\Periods_\alpha = \left\{ \int_\ell \alpha \BigMid \ell \in \Loops(X) \right\}$
\def\BigMid{\;\big|\;}
\def\BiggMid{\;\bigg|\;}

%% \Def : Domain of definition (for a partially defined map)
%% Usage: \Def(f) = { x ∈ X | f(x) is defined }
\newcommand{\Def}{\operatorname{def}}

%% \Val : Set of values (image) of a map
%% Usage: \Val(f) = { f(x) | x ∈ \Def(f) }
...
%% Exterior differential \der : \DForms^k(X) \to \DForms^{k+1}(X)
\newcommand{\der}{\operatorname{d}}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
%% MARK: Volume and Surface forms
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% A chosen volume form of a manifold $M$ of any dimension.
%% If $\dim(M) = n$ then $\Vol \in \DForms^n(M)$.
\newcommand{\Vol}{\operatorname{vol}}

%% The surface form of a 2-dimensional manifold.
%% This is a specific name for the volume form of a surface.
\newcommand{\Surf}{\operatorname{surf}}

%% The total area of a 2-dimensional manifold.
%% Usage $\Area(\Sigma) = \int_\Sigma \Surf$ where $\Sigma$ is a oriented surface.
\newcommand{\Area}{\operatorname{Area}}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
%% MARK: Chain-Homotopy Operator (path-primitive)
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% \Komega : path-primitive of ω (1-form on Paths(X))
\newcommand{\Komega}{\CHO\omega}

%% \Kalpha : path-primitive of α
\newcommand{\Kalpha}{\CHO\alpha}

%% \Kbeta : path-primitive of β
\newcommand{\Kbeta}{\CHO\beta}

%% \\KVol : path-primitive of \Vol
\newcommand{\KVol}{\CHO\Vol}

%% \Ktheta : path-primitive of \theta
\newcommand{\Ktheta}{\CHO\theta}
...
\newcommand{\Tr}{\operatorname{tr}}  %% Trace of a matrix or an operator

%% \OrbitMap : Orbit map of a group action
%% Usage: \OrbitMap{x}(g) = g(x)
\newcommand{\OrbitMap}[1]{\hat{#1}}

%% \ActionGraph : Graph map of a group action
%% Usage: \ActionGraph(x,g) = (x, g \cdot x)
%% For each fixed g, x ↦ (x, g·x) is the graph of the diffeomorphism x ↦ g·x.
\newcommand{\ActionGraph}{\operatorname{graph}}

%% The identity of a group
%% Usage: \id_G, the identity of the group G
...
%% \Groupoid : Typesets the name of a diffeological groupoid in bold
%% Usage: \Groupoid{T} produces \mathbf{T}
%%        \Groupoid{G} produces \mathbf{G}
%% Mathematical meaning: A diffeological groupoid, typically
%%   - \Groupoid{T}_\omega : the prequantum groupoid
%%   - \Groupoid{H} : the homotopy groupoid
%%   - \Groupoid{G} : a generic groupoid
%%
%% For calligraphic groupoids (e.g., \mathcal{G} for a category),
%% use \cG, \cH, etc.
\newcommand{\Groupoid}[1]{\mathbf{#1}}
...
%% \EndsPath : The pair of endpoints of a path (value in X × X)
%% Usage: \EndsPath(\gamma) = (\0(\gamma), \1(\gamma))
\def\EndsPath(#1){(\0(#1),\1(#1))}

%% \Ends : Short alias for \EndsPath
\let\Ends\EndsPath

%% \EndsMap : The ends map as a smooth morphism
%% Usage: \EndsMap : \Paths(X) → X × X is a subduction
\newcommand{\EndsMap}{\operatorname{ends}}

%% \tValue : Evaluation at time t
%% Usage: \tValue(\gamma) = \gamma(t) for t \in \RR
\newcommand{\tValue}{\hat{t}}
...
%% \ConstPath : The constant path (or constant loop) at a given point
%% Type: Point → Path
%% Usage: \ConstPath{x} produces \hat{x} (the constant path t ↦ x)
%% Semantics: Allows changing the notation of constant paths (e.g., to c_x or \mathbf{x}) globally
\newcommand{\ConstPath}[1]{\hat{#1}}
...
\newcommand{\Domains}{\operatorname{Domains}}
...
%% \GenFam : Generating family of a diffeology
\newcommand{\GenFam}{\mathrm{Gen}}
...
%% \Closure : Topological closure of a set
%% Usage: \Closure{A} = \overline{A}
\newcommand{\Closure}[1]{\overline{#1}}

%% \Interior : Topological interior of a set
%% Usage: \Interior{A} = \operatorname{int}(A)
\def\Interior(#1){\operatorname{int}(#1)}

%% \Exterior : Topological exterior of a set
%% Usage: \Exterior{A} = \operatorname{ext}(A)
\def\Exterior(#1){\operatorname{ext}(#1)}

%% \Frontier : Topological frontier (boundary) of a set
%% Usage: \Frontier{A} = \partial A
\newcommand{\Frontier}{\partial}
...
%% \Proj : Projection map with optional custom symbol
%%
%% SEMANTICS: Canonical projection map from a space to its quotient.
%%
%% USAGE:
%%   \Proj               → pr
%%   \Proj[\pi]          → π
...
%%
%% EXAMPLES:
%%   $\Proj : X \to X/{\sim}$          produces "pr : X → X/∼"
%%   $\Proj[\pi]_\alpha : T^2 \to \cT_\alpha$   produces "π_α : T² → T_α"
%% or
%%   $\Proj[\pi_\alpha] : T^2 \to \cT_\alpha$   produces "π_α : T² → T_α"
\makeatletter
\def\Proj{\@ifnextchar[{\@Proj}{\@Proj[\operatorname{pr}]}}
\def\@Proj[#1]{#1}
\makeatother

%% \pr : Shortcut for \Proj
\let\pr\Proj
...
\def\FundamentalGroup(#1){\pi_1(#1)}

%% \piX : Alternative semantic name (shorter, still clear)
%% Usage: \piX{X} produces π₁(X)
\newcommand{\piX}[1]{\pi_1(#1)}

%% \HomotopyGroup : The i-th homotopy group
%% Usage: \HomotopyGroup_i produces π_i
%% \HomotopyGroup_i(,x_0) produces π_i(X,x_0)
\def\HomotopyGroup{\pi}
...
%% Specific periods for a closed 2-form \omega
\newcommand{\Pomega}{\Periods_\omega}

%% Specific torus of periods for a closed 2-form \omega
\newcommand{\Tomega}{\Torus_\omega}

%% Group of spherical periods for a closed 2-form \omega
\newcommand{\Psph}{\Periods_{\mathrm{sph}}}

%% Group of toric periods for a closed 2-form \omega
\newcommand{\Ptor}{\Periods_{\mathrm{tor}}}

%% Group of surfacic periods for a closed 2-form \omega
\newcommand{\Psurf}{\Periods_{\mathrm{surf}}}

%% The torus of spherical periods for a closed 2-form \omega
\newcommand{\Tsph}{\Torus_{\mathrm{sph}}}

%% The torus of toric periods for a closed 2-form \omega
\newcommand{\Ttor}{\Torus_{\mathrm{tor}}}
...
%%  Custom Matrix Command equivalent to Vector but semantically specific
...
\newcommand{\VectSpan}[1]{\operatorname{span}(#1)}
...
%% Sans Serif letters (semantic: used for distinguish maps or sets sharing the same lette as name)
