%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
%%  PROJECT: On the Diffeology of Orbit Spaces
%%
...
\begin{document}
  ...
  He would also like to acknowledge the assistance provided by the AI assistants Gemini (Google) and DeepSeek (\begin{CJK}{UTF8}{gbsn}深度求索\end{CJK}).} % Added a thank you to Gemini
  ...
  \begin{abstract}
    We investigate the correspondence between the geometry of a smooth action
    of a compact Lie group on a manifold $\M$ and the intrinsic smooth structure
    of the orbit space $\M/\G$. The latter is captured by the \emph{Klein stratification},
    which partitions $\M/\G$ into orbits of local diffeomorphisms, classifying the space
    by its intrinsic singularity types. We show that this geometry maps coherently to
    the quotient through the \emph{isostabilizer decomposition} of $\M$, whose elements
    are the connected components of submanifolds where the stabilizer is constant.
    Our main result, the \emph{Correspondence Theorem}, establishes that the canonical
    projection induces a surjective map from this decomposition to the Klein strata.
    As a corollary, we define the \emph{Inverse Klein Stratification} on $\M$ by pullback,
    clarifying its relationship with the intrinsic geometry of the quotient.
  \end{abstract}
  ...
  As we will see,
  the intrinsic geometry of the quotient $\M/\G$ is assembled naturally according to three kinds of points,
  characterized by their local diffeological models:
  \begin{itemize}
    \item \textbf{Manifold points}, whose local model is a Euclidean space $\E$ (i.e., the effective stabilizer is trivial).
    These correspond to principal orbits.
    \item \textbf{Orbifold points}, whose local model is $\E/\Gamma$ where $\Gamma$ is a non-trivial finite group.
    These correspond to exceptional orbits.
    \item \textbf{Orthofold points}, whose local model is $\E/\H$ where $\H$ is a non-discrete compact Lie group.
    These correspond to non-exceptional singular orbits.
  \end{itemize}
  ...
  \begin{article}
    ...
    \begin{definition}{~(Stratification)}
      ...
      This is the minimal requirement for a partition to be called a stratification;
      we therefore call it a \emph{basic stratification} and label it with the condition:
      \begin{itemize}
        \item[{[B]}] The closure of every stratum $\S \in \cS$ is a union of strata.
      \end{itemize}
    \end{definition}
  ...
  \begin{article}
    ...
    \begin{proposition}{~(Diffeological Local Model)}
      ...
      the quotient space $\M/\G$ is locally diffeomorphic to some quotient $\E/\H$ where $\E$ is a Euclidean space and $\H$ is a compact subgroup of $\O(\E)$ acting orthogonally.
      Precisely,
  ...
  \begin{article}
    ...
    \begin{definition}{~(Orthofolds)}%
      ...
      where $\E$ is a Euclidean vector space and $\H$ a subgroup of the orthogonal group $\O(\E)$.
  ...
  \begin{article}
    ...
    and consists of manifold points (i.e., points whose local model is a Euclidean space).
  ...
  \begin{article}
    ...
    This is precisely why this decomposition — which records not just the stabilizer but also the slice representation via its connected components — is necessary to understand the quotient's intrinsic structure.
    ...
    These examples collectively motivate the need for a partition of $\M$,
    which will be defined in the following article,
    that is sensitive not only to the stabilizer of a point,
    but also to the specific slice representation.
    This is precisely what this decomposition captures,
  ...
  \begin{article}
    ...
    This motivates the need for a finer partition of $\M$ that is sensitive to this additional local structure.
  \end{article}
  ...
  \begin{article}
    ...
    \begin{proposition}{~(Geometric Nature of Quotient Points)}%
      ...
      \begin{itemize}
        ...
        then $\cO$ is a \textbf{manifold point} (i.e., its local model is a Euclidean space).
        \item[$\ast$] If $x$ is an exceptional singular point (i.e., $\dim(\K) = \dim(\K_{\pr})$ but $\K$ is not conjugate to $\K_{\pr}$),
        then $\cO$ is a non-trivial \textbf{orbifold point} (i.e., its local model is $\E/\Gamma$ where $\Gamma$ is a non-trivial finite group).
        \item[$\ast$] If $x$ is a non-exceptional singular point (i.e., $\dim(\K) > \dim(\K_{\pr})$),
        then $\cO$ is a \textbf{general orthofold point} (i.e., its local model is $\E/\K$ where the effective action of $\K$ is by a non-discrete compact Lie group).
      \end{itemize}
    \end{proposition}
