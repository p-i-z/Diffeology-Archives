...
\begin{document}
  ...
  \title[Bosonic and Fermionic Singularities]{Bosonic and Fermionic Singularities\\ in Diffeology}
  ...
  \subjclass[2020]{Primary 58A40; Secondary 58A10, 58A32, 53A45, 58K70.}
  ...
  diffeology tests the space via smooth parametrizations (plots).
  ...
  \begin{article}\artlabel{Symmetric Tensors}
    ...
    Let $\P \colon \U \to \Delta$ be a plot.
    We must verify that $\P^*(\tau_\sing) = (\P'(t)^2/\P(t)) \dt \otimes \dt$ is smooth.
    If $\P(t_0) > 0$,
    smoothness is trivial.
    If $\P(t_0) = 0$,
    we rely on the following classical inequality.
    ...
    Applying this to our plot $\P$,
    we have $|\P'(t)^2/\P(t)| \le 2 \sup |\P''|$.
    If $\P$ is flat at $t_0$,
    then $\P''$ vanishes as we approach $t_0$,
    so the ratio tends to $0$.
    Differentiation of the ratio leads to similar expressions involving higher derivatives,
    which also vanish.
    Thus,
    the pullback is smooth everywhere.
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %% MARK: Riemannian Metrics
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \section*{Riemannian Metrics}
  
  \begin{article}\artlabel{Definition and Exclusion of Singularities}
    We adopt the definition of a Riemannian metric proposed in \cite[p.~224]{PIZ25}.
    A \emph{Riemannian metric} on a diffeological space $\X$ is a smooth,
    symmetric and
    positive definite covariant 2-tensor $g$ in the following sense:
    
    \begin{enumerate}
      \item For all $x \in X$ and all paths $\gamma \in \Paths(X)$ pointed at $x$ (i.e. $\gamma(0) = x$), $g(\gamma)_0(1,1) \geq 0$.
      \item For every $x \in \X$ and every plot $\gamma$ centered at $x$:
      $$
        g(\gamma)_0(1)(1) = 0 \iff \forall \alpha \in \Lambda_x^1(\X), \ \alpha(\gamma)_0(1) = 0,
        $$
      where $\Lambda_x^1(\X)$ denotes the space of pointed 1-forms at $x$.
      \end{enumerate}
      
      Applying this to the decomposition $\tau = c \tau_\sing + \tau_\reg$ on the half-line, we see that the singular part must vanish for a metric.
      Consider the plot $\gamma(t) = t^2$.
      It is centered at $0$, and for any 1-form $\alpha = f(x)\dx$,
      we have $\alpha(\gamma) = f(t^2) 2t \dt$,
      which vanishes at $t=0$.
      Thus,
      the "definite" condition requires $g(\gamma)_0 = 0$.
      However, evaluating the singular tensor on this path gives:
      $$
        \gamma^*(\tau_\sing) = \frac{(2t \dt)^2}{t^2} = 4 \dt^2.
        $$
      This value is non-zero at $t=0$.
      Therefore,
      if $\tau$ is a Riemannian metric,
      we must have $c=0$.
      The "Bosonic" accumulation is incompatible with the non-degeneracy axiom of Riemannian geometry.
      
      \textbf{Remark.} The reader may notice that we define the metric and tensors directly via their action on plots (covariant tensors),
      using pointed $1$-forms as test "functions" for the "definite" condition,
      without reference to a specific definition of a "tangent vector" or "tangent bundle."
      This choice
      ---~discussed in the text book \cite[\textsection 6.53]{PIZ13}~---
      is deliberate.
      As demonstrated in a recent work by Masaki Taho \cite{Tah25},
      there exist infinitely many non-isomorphic tangent functors on diffeological spaces that extend the classical functor on manifolds.
      By working with the functional definition on plots,
      we avoid the ambiguity of selecting a specific tangent functor and rely solely on the intrinsic smooth structure defined by the plots.
    \end{article}
    ...
    \begin{thebibliography}{IZW13}
      ...
      \bibitem[GIZ17]{GIZ17}
      Serap Gürer and Patrick Iglesias-Zemmour.
       {\em Differential Forms on Corners}.
      Preprint, May 2017.\\
       \url{http://math.huji.ac.il/~piz/documents/DFOC.pdf}.
      ...
      {Patrick Iglesias-Zemmour.}
      {\em Diffeology.}
      {Mathematical Surveys and Monographs. The American Mathematical Society, vol. 185, USA R.I. 2013.}
      
      \bibitem[PIZ25]{PIZ25}
      {Patrick Iglesias-Zemmour.}
      {\em Lectures on Diffeology.}
      {Beijing World Publishing Corp., 2025.}
      
      \bibitem[IZW13]{IZW13}
      Patrick Iglesias-Zemmour and Enxin Wu.
      \newblock {\em A Few Half-Lines}.
      Web publication, April 21, 2013.\\
       \url{http://math.huji.ac.il/~piz/documents/DBlog-Rmk-AFHL.pdf}.
      
      \bibitem[Sou80]{Sou80}
      Jean-Marie Souriau.
      {\em Groupes diff{\'e}rentiels}.
       Lecture notes in mathematics, 836:91--128, 1980.
      
      \bibitem[Tah25]{Tah25}
      Masaki Taho.
       {\em Infinitely Many Tangent Functors on Diffeological Spaces.}
       Preprint, 2025 \url{https://arxiv.org/abs/2511.17871}.
      
      \bibitem[Whi43]{Whi43}
      Hassler Whitney.
      {\em Differentiable even functions.}
      {Duke Math. Journal, pp. 159--160, vol. 10, 1943.}
