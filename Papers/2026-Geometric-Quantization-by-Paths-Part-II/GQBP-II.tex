...
\begin{document}
  ...
  \begin{article}\artlabel[The Chasles Cocycle]
    ...
    $$
      ...
      $$
    The term for $L$ vanishes because the loop is constant with respect to $s$.
    The term for $\delta_s$ involves integrating $\omega$ over the map $\Phi(s,t) = \delta_s(t)$.
    Since $\Phi$ factors through the smooth curve $\delta$,
    its pullback $\Phi^*\omega$ vanishes identically.
    The same applies to $\bar\delta_s$.
    Thus, $\int_\sigma \Komega = 0$.
    
    Since $\psi$ is defined by integrating $\Komega$ along paths in loop space,
