...
\begin{document}
  ...
  \begin{abstract}
    ...
    The classical space $X$ is embedded as the \emph{skeleton} of units, surrounded by a \emph{quantum fog} of morphisms.
  ...
  \begin{article}\artlabel[The Central Extension]
    ...
    $$
      ...
      $$
    \textbf{Remark.} This extension is \emph{central}.
  ...
  \begin{article}\artlabel[The Total Group of Periods]
    ...
    For any relation $w \in \mathcal{R}$, we define $\mathcal{T}(w)$ by the condition:
    $$
      \Psi(w) = (1, \mathcal{T}(w)).
      $$
  ...
  \begin{proof}
    ...
    \textbf{1. Construction of the Deformation Path.}
    ...
    The construction is defined by the pointwise concatenation of the loops in $X$:
    $$
    ...
    \textbf{2. The Cycle in Loop Space.}
...
\textbf{Independence of $\delta$.}
    The function $\psi$ is defined on the space of free loops $\Loops(X)$.
    Consider the loop $\Lambda = \delta \vee L \vee \bar\delta$ based at $x_0$, and the loop $L$ based at $x = \gamma(0)$.
    Let $\delta_s$ be the path $t \mapsto \delta(s + t(1-s))$, which retracts $\delta$ to $x$.
    Consider the homotopy $\sigma: s \mapsto \delta_s \vee L \vee \bar\delta_s$ in $\Loops(X)$, which connects $\Lambda$ to $L$.
    The integral of $\Komega$ along $\sigma$ decomposes into three terms:
    $$
      \int_\sigma \Komega = \int_{s \mapsto \delta_s} \Komega + \int_{s \mapsto L} \Komega + \int_{s \mapsto \bar\delta_s} \Komega.
    $$
    The term for $L$ vanishes because the plot is constant.
    The term for $\delta_s$ involves integrating $\omega$ over the map $(s,t) \mapsto \delta_s(t)$.
    This map factors through the 1-plot $\delta$.
    Since the pullback of a 2-form by a 1-plot is identically zero, this integral vanishes.
    The same applies to $\bar\delta_s$, thus $\int_\sigma \Komega = 0$.
    
    Since $\psi$ is defined by integrating $\Komega$ (modulo $\Pomega$) from a reference loop,
    and the integral along the path $\sigma$ connecting $\Lambda$ to $L$ is zero,
    it follows that $\psi(\Lambda) = \psi(L)$.
    Therefore, the value $\Phi(\gamma, \gamma')$ is intrinsic.
  ...
  \begin{article}\artlabel[The Quantum Broth]
    ...
    This infinite-dimensional space contains every possible history of the system.
  ...
  \begin{article}\artlabel[Definition of the Groupoid]
    ...
    \begin{definition}{~(The Prequantum Groupoid)}
      ...
      $$
        ...
        $$
      The composition is induced by the concatenation of paths: $[\gamma]_\omega \cdot [\gamma']_\omega = [\gamma \vee \gamma']_\omega$.
    \end{definition}
    
    This object unifies the prequantum geometry, as illustrated below.
  ...
  \begin{article}\artlabel[Structural Properties]
    ...
    \begin{theorem}{~2}
      ...
      \begin{enumerate}
        ...
        \item There exists a unique left-right invariant $1$-form $\blambda$ on $\cY$ such that $\class_\omega^*(\blambda) = \Komega$.
  ...
  \begin{article}\artlabel[The Quantum System]
    ...
    The non-unit morphisms appear as a \emph{quantum fog} surrounding the skeleton of classical motions,
