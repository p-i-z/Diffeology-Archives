...
\begin{document}
  ...
  \begin{proof}
    ...
    \emph{1. Construction of the Deformation Path.}
    ...
    \emph{2. The Cycle in Loop Space.}
...
\emph{4. Independence from the Basepoint.}
    ...
    Let $c_s$ be the path $t \mapsto c(s + t(1-s))$, which retracts the path $c$ to the point $x_0$ as $s$ goes from $0$ to $1$.
    ...
    $$
      \phi(L) - \phi(\ell) = \int_\sigma \Komega.
    $$
    This integral decomposes into three terms corresponding to the three segments of the concatenated loop.
    The term for the central loop $\ell$ vanishes because it is constant with respect to the homotopy parameter $s$.
    The term for the segment $c_s$ involves integrating $\omega$ over the map $\Psi(s,t) = c_s(t)$.
    Since $\Psi$ factors through the smooth curve $c$,
    that is,
    a $1$-plot,
    the term $c_s$ vanishes identically.
  ...
  \begin{article}\artlabel[The Chasles Cocycle]
    ...
    \begin{proposition}[Independence of\/ $\delta$]
