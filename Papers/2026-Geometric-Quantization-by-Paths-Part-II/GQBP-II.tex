...
\textbf{Independence of $\delta$.}
    Let $L = \gamma \vee \bar\gamma'$. Let $\delta$ and $\delta'$ be two paths connecting $x_0$ to $\gamma(0)$.
    Let $\sigma = \delta' \vee \bar\delta$, which is a loop based at $x_0$.
    Consider the concatenation $\sigma \vee \delta = (\delta' \vee \bar\delta) \vee \delta$.
    The homotopy from $\sigma \vee \delta$ to $\delta'$ consists of retracting the segment $\bar\delta \vee \delta$ to a point that sweeps no area.
    Consequently, we have the strict equality of integrals (not just modulo periods), allowing us to write:
    \begin{align*}
      \psi(\delta' \vee L \vee \bar\delta')
      &= \psi\big( (\sigma \vee \delta) \vee L \vee \overline{(\sigma \vee \delta)} \big) \\
      &= \psi\big( \sigma \vee (\delta \vee L \vee \bar\delta) \vee \bar\sigma \big) \\
      &= \psi(\sigma) + \psi(\delta \vee L \vee \bar\delta) - \psi(\sigma) \\
      &= \psi(\delta \vee L \vee \bar\delta).
    \end{align*}
    Thus, the value $\Phi(\gamma, \gamma')$ is intrinsic.
