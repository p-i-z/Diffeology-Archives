...
\begin{document}
  ...
  \begin{article}\artlabel[The Chasles Cocycle]
    ...
    Since $\Tomega$ is an abelian group (as a quotient of $\RR$),
    the function $\psi$ is invariant under conjugation.
    For any loop $L$ and path $\delta$:
    $$
      \psi(\delta \vee L \vee \bar\delta) = \psi(\delta) + \psi(L) - \psi(\delta) = \psi(L).
      $$
    This abelian property is guaranteed because $\Pomega$ contains the symplectic areas of the commutators of $\pi_1(X)$,
    forcing them to vanish in the quotient $\Tomega$.
    Thus,
    the value $\Phi(\gamma, \gamma')$ is intrinsic and independent of the choice of $\delta$.
    
    This function satisfies the Chasles relation for any triple of paths with matching endpoints:
    $$
